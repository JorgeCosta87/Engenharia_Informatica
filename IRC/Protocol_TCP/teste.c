...
- As mensagens de número ímpar de cada cliente serão enviadas apenas aos clientes par
 (cliente 2, cliente 4, ...) e as mensagens de número par de cada cliente serão apenas enviadas 
 aos clientes impar (cliente 1, cliente 3, ...)
===================================================================================
*/
...
int main(int argc,char *argv[]){
 ...
 // if(sock=socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALIDA_SOCKET)
    ...
    //  if(contador >= 4){		
      ...
      //  tempoEspera.tv_usec = 0;
		...
		if(!ocorrenciaTimeout){
         ...
         // if(newSock=accpet(sock,(struct sockaddr * )&cli_addr, &cliaddr_len)) == SOCKET_ERROR){2
			...
			/*...*/
            send(newSock, MSG_BOAS_BINDAS, strlen(MSG_BOAS_BINDAS), 0);
...
void AtendeCliente(LPVOID param){
 ...
 //         FD_SET(grupo[i], &fdread);
	...
	while(1){
		...
		switch(/*..*/){ // Sem timeout
		//switch(select(32, &fdtemp, NULL, NULL, NULL)){
			case SOCKET_ERROR:
				if(WSAGetLastError()==WSAEINTR)
					break;
...
int difunde(SOCKET origem, SOCKET *grupo, int tam, int sock_number)
{
 ...
 // if((result=recv(origem, &c, sizeof(char), 0))==sizeof(char)){   
		   ...
		   for(i=0; i<tam; i++){
			   if(origem!=grupo[i]){
	// o) teste se o cliente destino é impar e se o número da mensagem a enviar é par e envie os bytes recebidos
					if(/*...*/ && /*...*/ ){
                  //if((i % 2) != 0 && (n_msg[i] % 2) == 0)
                		result=send(grupo[i], (char *)c,sizeof(c), 0);
						
					}
	//p) teste se o cliente destino é par e se o número da mensagem a enviar é impar e envie os bytes recebidos				
					if(/*...*/ && /*...*/ ){
                 // if((i % 2) == 0 && (n_msg[i] % 2) != 0)     
						result=send(grupo[i],(char *)c,sizeof(c), 0);
